...
pygame.display.set_caption("MR AURA ")
clock = pygame.time.Clock()

# --- MQTT Configuration ---
MQTT_BROKER = "192.168.0.157"  
MQTT_PORT = 1883
MQTT_TOPIC_PADDLE_CONTROL = "pong_Qossay" 


# 0: No move, 1: Move Up, -1: Move Down
mqtt_paddle_command = 0
...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        client.subscribe(MQTT_TOPIC_PADDLE_CONTROL)
        print(f"Subscribed to topic: {MQTT_TOPIC_PADDLE_CONTROL}")
    else:
...
def on_message(client, userdata, msg):
    ...
    command = int(payload)
    if command in [-1, 0, 1]:
        mqtt_paddle_command = command

    else:
        print(f"Invalid MQTT message payload: {payload}. Expected 0, 1, or -1.")
        
# --- End MQTT Callback Functions ---
...
try:
    ...
    print(f"MQTT connection refused. Check if broker {MQTT_BROKER} is running and accessible.")
except Exception as e:
    print(f"Failed to connect to MQTT broker {MQTT_BROKER}:{MQTT_PORT}. Error: {e}")
    print("Paddle will not be controllable via MQTT.")
...
while running:
    ...
    if current_state == "PLAY":
        paddle_speed = 7 # Define paddle speed
        if mqtt_paddle_command == 1:  # 1 for UP
            paddle.move(-paddle_speed)
        elif mqtt_paddle_command == -1: # -1 for DOWN
            paddle.move(paddle_speed)
        ...
        if ball.rect.colliderect(paddle.rect):
            ...
            ball.dx = abs(ball.dx)
            ball.dy = -relative_y # Corrected dy direction for bounce

            length = math.hypot(ball.dx, ball.dy)
            if length > 0: # Avoid division by zero
                ball.dx = (ball.dx / length) * DIFFICULTIES.get(pygame.K_1 if not selected_level else [k for k,v in DIFFICULTIES.items() if v['label'] == selected_level][0] , {'speed': ball.speed})['speed'] / ball.speed # Normalize dx
                ball.dy = (ball.dy / length) * DIFFICULTIES.get(pygame.K_1 if not selected_level else [k for k,v in DIFFICULTIES.items() if v['label'] == selected_level][0] , {'speed': ball.speed})['speed'] / ball.speed # Normalize dy
    ...
    if current_state == "ENTER_NAME":
        ...
        try:
            ...
            for i, score_entry in enumerate(highscores): # Renamed 'score' to 'score_entry' to avoid conflict
                y_pos = 190 + i * 30
                text = f"{i+1}. {score_entry['name'][:8]} - {format_time(score_entry['best_time'])}"
        ...
        try:
            ...
            for i, score_entry in enumerate(highscores[:10]): # Renamed 'score' to 'score_entry'
                y_pos = 80 + i * 30
                text = f"{i+1}. {score_entry['name']} - {score_entry['level']}"
                show_text(text, COLORS['WHITE'], 420, y_pos, 18, False)
                show_text(f"Best: {format_time(score_entry['best_time'])}", COLORS['WHITE'], 580, y_pos, 18, False) # Corrected to 580 to fit
        except:
